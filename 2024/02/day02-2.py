    ...
    for (line, data) in enumerate(file):
        for report in data.splitlines():
            if is_safe(report):
                ...
                # Brute force the report until it is safe, it at all. Fancier
                # methods didn't give the correct result, so we'll stick with
                # that.
                report = report.split()
                for i in range(len(report)):
                   r = ' '.join([n for (idx, n) in enumerate(report) if idx != i])
                   if is_safe(r):
                       safe_reports += 1
                       break
...
class TestReports(unittest.TestCase):
    ...
    def test_reports(self):
        self.assertEqual(self.safe_reports, 4)
